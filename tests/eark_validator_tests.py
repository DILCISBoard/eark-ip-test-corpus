...
def eark_validator_test_from_test_case(test_case: case.TestCase, path_to_test_case_directory: Path):
    for rule in test_case.rules.rule:
        for package in rule.corpus_packages.package:
            ...
            compare_output_with_expected_value_from_test_case(output, package.is_valid, test_case.id.requirement_id)

def validate_package_with_eark_validator(path: Path) -> str:
    return subprocess.check_output(['eark-validator', path], text=True)

def compare_output_with_expected_value_from_test_case(output: str, is_package_valid: case.PackageIsValid, requirement_id: str):
    return compare_output_with_expected_value(output, is_package_valid == case.PackageIsValid.TRUE, requirement_id)

def compare_output_with_expected_value(output: str, should_contains_requirement: bool, requirement_id: str):
    ...
    if should_contains_requirement:
        assert is_structure_well_formed
    else:
        if not is_structure_well_formed:
            assert any(result['rule_id'] == requirement_id for result in json_data["structure"]["messages"])
            return
    ...
    if should_contains_requirement:
        assert not (is_requirement_in_structure_results and is_requirement_in_schematron_results and is_requirement_in_file_results)
    else:
        assert (is_requirement_in_structure_results or is_requirement_in_schematron_results or is_requirement_in_file_results)